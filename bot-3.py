...
# Фикс ошибки audioop (игнорируем)
try:
    import audioop
except ImportError:
    pass
...
print(f"📝 Спам текст: {spam_text[:50]}..." if len(spam_text) > 50 else f"📝 Спам текст: {spam_text}")
...
async def kill(ctx):
    ...
    print(f"💀 Уничтожение сервера {guild.name}...")
    ...
    print(f"✅ Создано {len(created_channel_ids)} каналов. Спам запущен!")
...
async def stop(ctx):
    ...
    print(f"🛑 Бот остановлен на сервере {guild.name}")
